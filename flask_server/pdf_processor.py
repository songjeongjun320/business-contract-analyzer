        ...
        with open(input_pdf_path, 'rb') as pdf_file:
            ...
            for i in range(num_pages):
                ...
                if page_text:
                    ...
                    processed_txts.append(txt_filename)
                    extracted_texts[page_num] = page_text
                else:
                    print(f"[WARNING] 페이지 {i + 1}에서 텍스트를 추출할 수 없습니다.")

        print("[INFO] PDF 분할 및 텍스트 추출 완료.")
        return "PDF splitting and text extraction completed successfully.", processed_pdfs, processed_txts, extracted_texts
